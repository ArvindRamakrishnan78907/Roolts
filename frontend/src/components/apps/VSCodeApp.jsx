...
const VSCodeApp = ({ onBack }) => {
        ...
        try {
            ...
            if (!installData.downloadUrl) {
                ...
                if (searchRes.ok) {
                    ...
                    const match = data.extensions?.find(e => `${e.namespace}.${e.name}` === id);
    ...
    const testCompiler = async (lang) => {
        addNotification(`Testing compiler integration for ${lang}...`, 'info');
        ...
        try {
            ...
            if (result.success) {
                ...
                addNotification(`${lang} compiler is working!`, 'success');
            } else {
                setError(result.error);
                addNotification(`${lang} compiler test failed`, 'error');
            }
        } catch (error) {
            setError(error.message);
            addNotification(`Test execution failed: ${error.message}`, 'error');
        } finally {
            setExecuting(false);
        }
    };
